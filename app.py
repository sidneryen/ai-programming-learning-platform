        ...
        try:
            ...
            if user:
                ...
                try:
                    ...
                    # check_password_hash 可能会抛出异常（如果密码不是有效的哈希格式）
                    pass
                
                # 如果加密验证失败，检查是否是明文密码（兼容历史数据）
                if not password_valid:
                    # 密码长度小于100很可能是明文存储
                    if len(user.password) < 100 and user.password == password:
                        password_valid = True
                        # 可选：自动将明文密码更新为加密格式
                        from werkzeug.security import generate_password_hash
...
def reset_all_student_passwords():
    ...
    try:
        ...
        try:
            ...
            return jsonify({
                "success": True,
                "message": f"成功重置 {reset_count} 名学生的密码为 123456"
            })
        finally:
            session_db.close()
    except Exception as e:
